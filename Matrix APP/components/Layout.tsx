...
const Layout: React.FC<LayoutProps> = ({ user, children, onLogout, activeView, setActiveView }) => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto pb-20 md:pb-0">
        <header className="h-16 bg-white border-b flex items-center justify-between px-4 md:px-8 sticky top-0 z-10">
          ...
          <div className="flex items-center gap-4">
             <span className="text-xs md:text-sm text-slate-500 hidden sm:inline">{new Date().toDateString()}</span>
             <button onClick={onLogout} className="md:hidden p-2 text-slate-400 hover:text-red-600 transition-colors">
                <LogOut size={20} />
             </button>
          </div>
        </header>
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex items-center justify-around h-16 px-2 z-20">
        {navItems.map((item) => (
          <button
            key={item.id}
            onClick={() => setActiveView(item.id)}
            className={`flex flex-col items-center justify-center gap-1 flex-1 h-full transition-colors ${
              activeView === item.id ? 'text-teal-600' : 'text-slate-400'
            }`}
          >
            <item.icon size={20} />
